...
MONGODB_COLLECTION_DRIVERS = os.environ.get("MONGODB_COLLECTION_DRIVERS")
...
def index():
    ...
    return render_template('driver_homepage.html', map_content=map_obj._repr_html_(), open_orders=open_orders, drivers=drivers)
...
def details(order_id):
    ...
    if request.method == 'POST':
        ...
        flash("Order received successfully!")
        return redirect(url_for('driver_blueprint.details', order_id=order_id))

    return render_template('ride_details.html', order=order)
...
def display_invoice(order_id):
    ...
    return render_template('driver_invoice.html', order=order, user=user, driver=driver)
...
def send_invoice_email(recipient, order, user, driver):
    ...
    sender = 'your_company_email@example.com'  # Update with your company email
...
def view_profile():
    ...
    return render_template('driver_profile.html', driver_info=driver_info)
